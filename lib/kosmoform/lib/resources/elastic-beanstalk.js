...
var ConfigurationTemplate = AWSResource.define('AWS::ElasticBeanstalk::ConfigurationTemplate', {
    ...
    validator: function(context) {
        if (!context.properties.EnvironmentId
            && !context.properties.SolutionStackName
            && !context.properties.SourceConfiguration) {
            context.addError('Must specify either EnvironmentId, SolutionStackName, or SourceConfiguration');
        }
    },
});
...
var Environment = AWSResource.define('AWS::ElasticBeanstalk::Environment', {
    ...
    validator: function(context) {
        if (context.properties.EnvironmentName) {
            context.addWarning('Warning: specifying EnvironmentName (' + context.properties.EnvironmentName + ') will disallow updates that require replacement.');
        }
    },
});

...
var Cluster = AWSResource.define('AWS::Redshift::Cluster', {
    ...
    ClusterSecurityGroups : { type: types.array(types.string) },
    ...
    VpcSecurityGroupIds : { type: types.array(types.string) },
}, {
    validator: function(context) {
        if (context.properties.NumberOfNodes && context.properties.ClusterType === 'single-node') {
            context.addError('NumberOfNodes must be unset if NodeType is single-node');
        }
    },
});

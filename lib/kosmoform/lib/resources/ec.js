...
var CacheCluster = AWSResource.define('AWS::ElastiCache::CacheCluster', {
    ...
    validator: function(context) {
        if (context.properties.PreferredAvailabilityZones && !context.properties.AZMode) {
            context.addError('AZMode must be set if PreferredAvailabilityZones is set (notice the \'s\' at the end');
        }

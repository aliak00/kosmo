...
Command.prototype.execute = function() {
    ...
    var componentPromises = _.map(this.project.components(), component => {
    ...
    return Promise.all(componentPromises).then(values => {
        ...
        if (this.project.components().length) {
            console.log('\n== Components ==\n');
        }

        var stacks = utils.zipObject(this.project.components(), values);
        ...
        var artifactPromises = _.map(this.project.artifacts(), artifact => {
        ...
        if (this.project.artifacts().length) {
            console.log('\n== Artifacts ==\n');
        }

        var artifacts = utils.zipObject(this.project.artifacts(), values);

...
function Command(opts) {
    ...
    if (!program.initialized() && this.dataBucket) {
        throw new Error('Please run kosmo init before creating a data-bucket.');
    }

    if (program.initialized() && !this.dataBucket) {
        throw new Error('Kosmo already seems to be initialized for profile ' + program.getProfile());
    }
    ...
    if (this.dataBucket) {
        if (program.getKosmoBucket().region === this.options.region) {
...
Command.options = [
    ['', 'data-bucket', 'Tells kosmo that you are initializing a data bucket.'],
    ['', 'bucket-name', 'What you want to call the bucket being created'],
    ['', 'region=ARG', 'Which region to initialize the kosmo bucket in'],
];
Command.usageText = '[options]';
Command.descriptionText = 'Initialize kosmo for an aws account';
...
Command.prototype.execute = function() {
    ...
    return getUser().then((userData) => {
        ...
        var bucketName = this.bucketName || util.format('kosmo-%s-%s', this.region, awsAccountId);
        if (!this.bucketName) {
        ...
        if (program.options.verbose) {
            console.log('Creating kosmo bucket');
        }
        ...
        if (err.code === 'BucketAlreadyOwnedByYou') {
            ...
            // Should not get here if kosmo config file and account is in sync

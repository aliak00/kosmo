...
Command.options = [
    ...
    // ['', 'region=ARG+', 'Explicitly specify regions if you want to overwrite the artifact definition regions'],
];
...
function ensureBucketsExist(regions) {
    var regionsWithoutBuckets = _.filter(regions, region => {
        return program.getKosmoBucket().region !== region
...
'Artifacts cannot be placed in regions without kosmo data buckets.\n\
You can ether run kosmo init --data-bucket and create the buckets\n\
or kosmo can create default buckets for you.\n\
Should kosmo create buckets in regions %s and carry on?\
', regionsWithoutBuckets.join(','));

    return utils.yesorno(question).then(shouldCreateBuckets => {
        if (!shouldCreateBuckets) {
            return Promise.reject(new KosmoError('Well then you don\'t get your artifact now. '
                + 'Please init buckets in region(s) ' + regionsWithoutBuckets.join(',') + '. See kosmo init.'));
        }
        ...
        var bucketsToCreate = _.map(regionsWithoutBuckets, region => {
            return {
                Bucket: util.format('kosmo-%s-%s', region, awsAccountId),
                ACL: 'private',
...
Command.prototype.execute = function() {
    ...
    artifactDef.region = this.commandOptions.region || artifactDef.region;
    if (!artifactDef.region) {
        throw new KosmoError('You must specifiy a region to build this artifact for.');
    }
    ...
    if (program.options.verbose) {
        console.log('Ensuring kosmo buckets in regions ' + metaInfo.regions.join(','));
    }

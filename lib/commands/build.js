...
function Command(opts) {
    ...
    this.artifactName = this.project.name;
}
...
function ensureBucketsExist(regions) {
    var regionsWithoutBuckets = _.filter(regions, region => {
        return program.getRootBucket().region !== region
            && program.getSecondaryBucket(region) === null;
    });
...
'Artifacts cannot be placed in regions without nova buckets.\n\
You can ether run nova init --secondary and create the buckets\n\
or nova can create default buckets for you.\n\
Should nova create buckets in regions %s and carry on?\
', regionsWithoutBuckets.join(','));

    return utils.yesorno(question).then(yesorno => {
        if (!yesorno) {
            return Promise.reject(new Error('Please init buckets in region(s) ' + regionsWithoutBuckets.join(',') + '. See nova init.'));
        }
        var userId = program.getUserId();
        // Params for aws.createBucket
        var bucketsToCreate = _.map(regionsWithoutBuckets, region => {
            return {
                Bucket: util.format('nova-%s-%s', region, userId),
                ACL: 'private',
        ...
        return Promise.all(createBucketPromises).then(values => {
            // Transform into objects accepted by Program.setSecondaryBucket
        ...
        return _.map(buckets, bucket => {
            return program.setSecondaryBucket(bucket);
        });
    });
}
...
Command.prototype.execute = function() {
    ...
    return ensureBucketsExist(metaInfo.regions).then(() => {
        ...
        var options = {}; // Currently unused but reserved for future use.
        return utils.pdone(artifactDef.build, artifactDef, options);
    }).then(artifact => {
        if (artifact.type !== Artifact.Type.EBApp) {
            throw new Error('Only EBApp artifact type supported currently');
        }
        ...
        var uploadInfo = _.map(metaInfo.regions, region => {
        ...
        metaInfo.artifacts = values;
        return Artifact.getMeta(this.project.name, this.artifactName).then(previousMeta => {
            if (previousMeta) {
                metaInfo.previousMeta = `meta/${this.project.name}/artifacts/${this.artifactName}/${previousMeta.timestamp}.json`;
            }

            return Artifact.setMeta(this.project.name, this.artifactName, metaInfo).then(() => {
                return previousMeta
            });
        }).then(previousMeta => {
            if (!previousMeta) {

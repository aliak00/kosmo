...
', regionsWithoutBuckets.join(','));
    ...
    return utils.yesorno(question).then(shouldCreateBuckets => {
        if (!shouldCreateBuckets) {
            return Promise.reject(new NovaError('Well then you don\'t get your artifact now. '
                + 'Please init buckets in region(s) ' + regionsWithoutBuckets.join(',') + '. See nova init.'));
        }
...
Command.prototype.execute = function() {
    ...
    if (!artifactDef.region) {
        throw new NovaError('You must specifiy a region to build this artifact for.');
    }

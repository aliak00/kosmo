...
Command.prototype.execute = function() {
    ...
    var stackInfoPromises = _.map(dependantComponents, componentName => {
        ...
        stackName = Component.makeStackName(this.project.name, dependantComponentDef.name);
    ...
    return Promise.all(stackInfoPromises).then(values => {
        ...
        var novaIdOutput = novaform.Output('novaId', this.buildResult.novaId);
        stack.add((buildResult.outputs || []).concat(novaIdOutput));
        ...
        return upload({
            ...
            return waitForStack(this.stackName, componentDef.region, status => {
                ...
                var cfn = new AWS.CloudFormation({ region: componentDef.region });
                return CfnStack.getStackInfo(cfn, Component.makeStackName(this.project.name, this.componentName));
            }).then(stack => {
                if (!program.options.verbose) {
                    return;
                }

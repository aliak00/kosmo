...
function ensureStackStateOk(stackName, region) {
    var cfn = new AWS.CloudFormation({ region: region });
    ...
    return Promise.all(stackInfoPromises).then(values => {
        ...
        return upload({
            ...
            return getObject().then(data => {
                ...
                if (previousMeta) {
                    metaData.previousMeta = `meta/${this.project.name}/templates/${this.componentName}/${previousMeta.timestamp}.json`;
                }
            ...
            return ensureStackStateOk(this.stackName, componentDef.region);
        }).then(status => {
            if (program.options.verbose) {
            ...
            var cfn = new AWS.CloudFormation({ region: componentDef.region });
            if (status === CfnStack.Status.DOES_NOT_EXIST) {
                var createStack = utils.pbind(cfn.createStack, cfn, {
                ...
                var updateStack = utils.pbind(cfn.updateStack, cfn, {
                    Capabilities: [ 'CAPABILITY_IAM' ], // TODO: Only use this is template validations says to
                    StackName: this.stackName,
                    TemplateURL: this.templateUrl,
                    Parameters: parameters,
            ...
            if (program.options.verbose) {
                console.log('Deployed stack id ' + data.StackId);
            }

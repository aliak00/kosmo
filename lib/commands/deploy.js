...
function fetchArtifactDependencies(artifactRefs) {
    var artifactPromises = _.map(artifactRefs, artifactRef => {
        ...
        return Artifact.getMeta(ref.name, ref.subname).then(meta => {
            if (!meta) {
                throw new NovaError('Could not find artifact ' + artifactRef + '. Have you built it?');
            }
...
Command.prototype.execute = function() {
    ...
    return fetchDependencies(this.project.getDependencies(this.componentDef.name), thisRef).then(dependencyData => {
        ...
        var stack = new novaform.Stack(this.stackName);
        ...
        var errorData = stack.validate();

        const allTheWarnings = _.flatMap(errorData.warnings, (warnings, resourceName) => {
            return _.map(warnings, warning => {
                return `resource "${resourceName}": ${warning}`;
            })
        });

        if (allTheWarnings.length) {
            console.log(`Found ${allTheWarnings.length} warnings:`);
            _.forEach(allTheWarnings, (warning, i) => {
                console.log(`  ${i + 1}) ${warning}`);
            })
        }

        var errorCount = 0;
        const allTheErrors = _.flatMap(errorData.errors, (errors, resourceName) => {
            return _.map(errors, error => {
                return `  ${++errorCount}) resource "${resourceName}": ${error}.`;
            });
        });

        if (errorCount) {
            throw new NovaError(`Found ${errorCount} errors:\n${allTheErrors.join('\n')}`);
        }

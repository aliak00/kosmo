...
function ensureValueValid(type, x) {
    if (!type.validate(x)) {
        throw Error(`${type.name}.toCloudFormationValue received invalid value ${x}.`);
    }
}
...
const Types = {
    ...
    ref: function(resourceType) {
        return {
            name: 'ref',
            // TODO: test that resourceType is correct as well.
            validate: function(x) { return x instanceof CloudFormationFunction.ref; },
    ...
    emptymap: {
        name: 'emptymap',
        validate: function(x) {
            return _.isObject(x) && _.isEmpty(x) && !_.isArray(x);
        },

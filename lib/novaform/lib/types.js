...
function ensureValueValid(type, x) {
    ...
    if (result) {
        throw Error(`${type.name}.toCloudFormationValue received invalid value ${x}: ${result}`);
    }
}
        ...
        return {
            ...
            validate: function(x, parentType) {
                ...
                if (diffMandatoryKeys.length > 0) {
                    errors.push(`${errorPrefix}mandatory properties ${mandatoryPropertyNames.join(', ')} not set`);
                }
                ...
                _.forEach(x, (propertyValue, propertyName) => {
                    ...
                    if (propertyDefinition.validators instanceof Array) {
                        _.forEach(propertyDefinition.validators, validator => {
                            var propertyValidationResult = validator(this);
                            if (!_.isUndefined(propertyValidationResult)) {
                                errors.push(`in ${typeName}.${propertyName} failed validation: "${propertyValidationResult}"`);
                            }
                        });
                    }
                });

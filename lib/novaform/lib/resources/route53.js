...
var HealthCheck = AWSResource.define('AWS::Route53::HealthCheck', {
    ...
    HealthCheckTags: { type: types.array(HealthCheckTagsType) },
});
...
var HostedZone = AWSResource.define('AWS::Route53::HostedZone', {
    ...
    HostedZoneTags: { type: types.array(HostedZoneTagsType) },
    Name: { type: types.string, required: true },
    VPCs: { type: types.array(HostedZoneVPCsType) },
});
...
var RecordSet = AWSResource.define('AWS::Route53::RecordSet', {
    ...
    ResourceRecords: { type: types.array(types.string), validators: [ResourceRecordsValidator] },
...
var RecordSetGroupRecordSetType = types.object('r53-record-set-group-record-set', {
    Name: types.string, // required
    Type: RecordSetTypeEnum, // required
    TTL: types.string,
    SetIdentifier: types.string, // conditional
    ResourceRecords: types.array(types.string), // required (because there's no AliasTarget here)
});

var RecordSetGroup = AWSResource.define('AWS::Route53::RecordSetGroup', {
    HostedZoneId: { type: types.string, required: 'conditional' },
    HostedZoneName: { type: types.string, required: 'conditional' },
    RecordSets: { type: types.array(RecordSetGroupRecordSetType), required: true },

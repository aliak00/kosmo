...
function Rds(options) {
    ...
    if (typeof password !== 'string' && !(password instanceof kosmoform.fn.ref)) {
        throw new Error('password must be either string or kosmoform.fn.ref');
    }
    ...
    this._addResource(kosmoform.ec2.SecurityGroupIngress(mkname('SgiPostgres'), {
    ...
    var subnetGroup = this._addResource(kosmoform.rds.DBSubnetGroup(mkname('PrivateSubnet'), {
        DBSubnetGroupDescription: name + ' db private subnets',
        SubnetIds: subnets,
        Tags: {
            Application: kosmoform.refs.StackId,
            Name: kosmoform.fn.join('-', [kosmoform.refs.StackName, mkname('PrivateSubnet')]),
        },
    }));

    var dbinstance = this._addResource(kosmoform.rds.DBInstance(mkname('Instance'), {
        ...
        Tags: {
            Application: kosmoform.refs.StackId,
            Name: kosmoform.fn.join('-', [kosmoform.refs.StackName, mkname('Instance')]),
        },
    }, {

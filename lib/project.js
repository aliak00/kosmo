...
'       );
    ...
    _.forEach(this.def.components, component => {
        if (!component.name) {
            throw new NovaError('Found component without a name. WHY YOU NO NAME COMPONENT!!??');
        }

        if (component.dependencies) {
            var dependencies = component.dependencies;
            if (!_.isObject(dependencies) || _.isArray(dependencies)) {
                throw new NovaError(`Expected '${component.name}' dependencies to be an object.`)
            }

            if (dependencies.components && !_.isArray(dependencies.components)) {
                throw new NovaError(`Expected '${component.name}' dependencies components to be an array.`)
            }

            if (dependencies.artifacts && !_.isArray(dependencies.artifacts)) {
                throw new NovaError(`Expected '${component.name}' dependencies artifacts to be an array.`)
            }

            var extraKeys = _.difference(_.keys(dependencies), ['artifacts', 'components']);
            if (extraKeys.length !== 0) {
                throw new NovaError(`Found unexpected nonsense inside '${component.name}' dependencies object: ${extraKeys.join(',')}`);
            }
        }
    });
};

...
function Artifact(type, pathToArtifact) {
    if (!(this instanceof Artifact)) {
        return new Artifact(type, pathToArtifact);
    }
...
Artifact.createLambdaZipFile = function(destinationName, sourcePath, options) {
    return Artifact.createZipFile(destinationName, sourcePath, options).then(artifact => {
        artifact.type = ArtifactType.Lambda;
        return artifact;
    });
};
...
Artifact.getMeta = function(projectName, artifactName) {
    ...
    var s3 = new AWS.S3({ region: kosmoBucket.region, signatureVersion: 'v4' });
...
Artifact.setMeta = function(projectName, artifactName, metaInfo) {
    ...
    return Artifact.getMeta(projectName, artifactName).then(previousMeta => {
        if (!previousMeta) {
            return;
        }

        metaInfo.previousMeta = `meta/${projectName}/artifacts/${artifactName}/${previousMeta.timestamp}.json`;
        return putObject({
            Bucket: kosmoBucket.name,
...
Artifact.find = function(region, projectName, artifactName) {
    return Artifact.getMeta(projectName, artifactName).then(info => {
        ...
        if (!artifact) {
            throw new KosmoError(`Could not find artifact '${artifactName}' in region ${region}.`);
        }
